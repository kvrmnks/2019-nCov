...
var rawData = d3.csvParse(readText('Area2.csv'));
...
connection.connect();
var sql = 'replace into city (provinceName,cityName,confirmedCount,curedCount,deadCount,updateTime,id) values (?,?,?,?,?,?,?)';
for(let i=0;i<rawData.length;i++){
    var arg = [];
    arg.push(rawData[i].provinceName);
    arg.push(rawData[i].cityName);
